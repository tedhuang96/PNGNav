...
from png_navigation.srv import GlobalReplan, GlobalReplanResponse
...
class GlobalPlanner:
    def __init__(
        ...
        rospy.Service('png_navigation/global_replan', GlobalReplan, self.global_replan)
    
    def global_replan(self, request):
        human_obstacles = request.replan_human_obstacles
        if len(human_obstacles) == 0:
            print("replan with no human neighbors.")
            is_replanned = self.plan()
            return GlobalReplanResponse(is_replanned)
        ...
        print("replan")
        print(env_with_humans.obs_circle)
        is_replanned = self.plan()
    ...
    def set_environment(self, env=None):
        ...
        try:
            ...
            request_env.x_range = env.x_range
            request_env.y_range = env.y_range
            if env.obs_circle is None:
                request_env.circle_obstacles = []
            else:
                request_env.circle_obstacles = np.array(env.obs_circle).flatten()
            if env.obs_rectangle is None:
                request_env.rectangle_obstacles = []
            else:
                request_env.rectangle_obstacles = np.array(env.obs_rectangle).flatten()
    ...
    def plan(self):
        ...
        try:
            ...
            plan_request.max_time = self.config.path_planner_args.max_time
